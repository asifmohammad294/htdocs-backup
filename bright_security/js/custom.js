...
------------------------------ */ 
    ...
    $(".submitForm").on("click", function() {
        ...
        var errroTarget = targetForm.find('.response').css('color', 'red');
        var check = checkRequire(targetForm , errroTarget);
        if(check == 0){
            $("#submit").html('Please Wait...').addClass('subBtn');
            ...
            $.ajax({
                method : 'POST',
                url : 'php/php pdf/contactForm.php',
                type: 'POST',
                data:{vname:name,vemail:email, vmessage:message}
            })
            .success(function(resp){
              ...
              //  console.log(res.data[0].error);
                if(res.data[0].error == "false"){ 
                   ...
                   $(".response").html("Congratulations! Your details has been submitted successfully.").css("color", "green");
                   setTimeout("$('.response').html(''); $('#submit').html('Submit').removeClass('subBtn'); $('#submit').attr('disabled', false);", 3000);
                }else{ 
                    console.log(res.data[0].error);
                    $('#submit').html('Submit').removeClass('subBtn'); 
                    $('#submit').attr('disabled', false);
                    $(".response").html("Error! Please try again.").css("color", "red");
                } 
            });
        }
    });
